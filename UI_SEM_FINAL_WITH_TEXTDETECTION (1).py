...
pytesseract.pytesseract.tesseract_cmd = r'C:\Users\gopubh\AppData\Local\Programs\Tesseract-OCR\tesseract.exe'
...
def process_image(image_path, pixel_to_um):
    try:
...
def process_file(root, file, base_dir, export_dir):
    try:
        image_path = os.path.join(root, file)
        pixel_to_um = calculate_pixel_to_um(image_path)
...
def traverse_and_process(base_dir, export_dir, max_files=2):
    ...
    with ThreadPoolExecutor(max_workers=24) as executor:
        ...
        for root, dirs, files in os.walk(base_dir):
            ...
            for file in md_files:
                futures.append(executor.submit(process_file, root, file, base_dir, export_dir))
...
def process_folder_and_subfolders(ctx, folder_url):
    ...
    if folder_name.lower() in ['top', 'bottom', 'core']:
        ...
        if png_files_list:
            ...
            export_folder_in_sharepoint = f"{folder_url}/ultimate_output2_{folder_name}"
            create_folder(ctx, folder_url, f"ultimate_output2_{folder_name}")
...
def calculate_pixel_to_um(image_path):
    def process_image_and_extract_text(image_path):
        ...
        bottom_part = image_cv[int(height * 0.94):int(height * 0.95), :]
        half_width = bottom_part.shape[1] // 2
        second_half = bottom_part[:, half_width:]

        gray_second_half = cv2.cvtColor(second_half, cv2.COLOR_BGR2GRAY)
        _, thresh_second_half = cv2.threshold(gray_second_half, 200, 255, cv2.THRESH_BINARY)

        return thresh_second_half

    def detect_vertical_bars_and_distances(image):
        ...
        binary = cv2.morphologyEx(binary, cv2.MORPH_OPEN, vertical_kernel)
        ...
        for contour in contours:
            ...
            if aspect_ratio > 2 and h > height * 0.05 and w < width * 0.1:
        ...
        if bounding_boxes:
            ...
            for i in range(len(bounding_boxes) - 1):
                ...
                distance = (x2 - (x1 + w1))
                total_distance += distance + w1
                mid_x1 = x1 + w1 // 2
                mid_y1 = y1 + h1 // 2
                mid_x2 = x2 + w2 // 2
                mid_y2 = y2 + h2 // 2
                cv2.line(contours_image, (mid_x1, mid_y1), (mid_x2, mid_y2), (255, 0, 0), 2)
                cv2.putText(contours_image, f'{distance}', ((mid_x1 + mid_x2) // 2, (mid_y1 + mid_y2) // 2), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                print(f"Distance between bar {i} and bar {i+1}: {distance} pixels")
            
            print(f"Total distance including bar widths: {total_distance} pixels")
            return total_distance, contours_image
        else:
            print("No vertical bars detected.")
            return None, image

    def get_pixel_to_um_factor(image, total_distance):
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        _, thresh = cv2.threshold(gray, 150, 255, cv2.THRESH_BINARY_INV)
        text_full = pytesseract.image_to_string(thresh)

        height, width = image.shape[:2]
        bottom_part = image[int(height * 0.95):, :]
        half_width = bottom_part.shape[1] // 2
        second_half = bottom_part[:, half_width:]
        gray_second_half = cv2.cvtColor(second_half, cv2.COLOR_BGR2GRAY)
        _, thresh_second_half = cv2.threshold(gray_second_half, 100, 255, cv2.THRESH_BINARY_INV)
        text_bottom = pytesseract.image_to_string(thresh_second_half)
        print(text_bottom)
        plt.figure(figsize=(10, 5))
        plt.imshow(cv2.cvtColor(thresh_second_half, cv2.COLOR_BGR2RGB))
        plt.title(f"Processed Image - {os.path.basename(image_path)}")
        plt.axis('off')
        plt.show()
        match = re.search(r'\d*\.?\d+(um|mm)', text_bottom)
        print(match)
        if match:
            text_um = match.group()
            print(text_um)
            scale_value = float(text_um.replace('um', '').replace('mm', '').strip())
            if 'mm' in text_um:
                scale_value *= 1000
            #print(scale_value)
            #print(scale_value / total_distance)
            return scale_value / total_distance
        return None

    processed_image = process_image_and_extract_text(image_path)
    total_distance, contours_image = detect_vertical_bars_and_distances(processed_image)

    if total_distance is not None:
        image = cv2.imread(image_path)
        if image is not None:
            pixel_to_um_factor = get_pixel_to_um_factor(image, total_distance)
            if pixel_to_um_factor is not None:
                print(f"Pixel to micrometer factor: {pixel_to_um_factor}")
                return pixel_to_um_factor
            else:
                print("Failed to extract pixel-to-um conversion factor.")
                return 2  # Default value
        else:
            print(f"Failed to load image {image_path}")
            return 2  # Default value
    else:
        print(f"No vertical bars detected in {os.path.basename(image_path)}.")
        return 2  # Default value
